...
const userSchema = new mongoose.Schema(
  {
    ...
    role: {
      type: String,
      enum: ["admin", "receptionist", "guest", "user"],
      default: "guest",
    },
...
export class UserRepositoryImpl {
  async save(user) {
    console.log("[USER REPOSITORY] Tentando salvar usuário:", {
    ...
    try {
      ...
      console.log("[USER REPOSITORY] Usuário salvo com sucesso:", {
        id: savedUser._id,
  ...
  async findById(id) {
    ...
    try {
      ...
      console.error("[USER REPOSITORY] Erro ao buscar usuário por id:", {
        message: error.message,
      });
      throw error;
    }
  ...
  async updateByUsername(username, data) {
    console.log("[USER REPOSITORY] Atualizando usuário por username:", {
      username,
      fields: Object.keys(data || {}),
    });
    try {
      const { password, ...rest } = data || {};
      const updated = await UserModel.findOneAndUpdate({ username }, rest, {
        new: true,
        runValidators: true,
      });
      if (!updated) {
  ...
  async deleteByUsername(username) {
    console.log(
      "[USER REPOSITORY] Tentando deletar usuário por username:",
      username
    );
    try {
      ...
      if (result.deletedCount > 0) {
        console.log(
          "[USER REPOSITORY] Usuário deletado com sucesso:",
          username
        );
        return true;
      } else {
        console.warn(
          "[USER REPOSITORY] Usuário não encontrado para deleção:",
          username
        );
        return false;
      }
