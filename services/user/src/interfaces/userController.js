...
// In test environment use a relaxed register schema so integration tests can
// create users with minimal fields (tests only send username/password/role).
let registerSchema = baseRegisterSchema;
...
export const register = async (req, res) => {
  try {
    ...
    const username = value.username && value.username.trim();
    ...
    console.log("[USER CONTROLLER] Usuário registrado com sucesso:", {
    ...
    res
      .status(201)
      .json({ mensagem: "Usuário criado com sucesso", usuario: dto });
  } catch (err) {
    ...
    res
      .status(status)
      .json({ erro: err.message || "Erro ao cadastrar usuário" });
  }
};
...
export const deleteUser = async (req, res) => {
  try {
    ...
    console.log(
      "[USER CONTROLLER] Recebendo requisição para deletar usuário:",
      username
    );
    ...
    if (!username || typeof username !== "string" || username.trim() === "") {
      return res
        .status(400)
        .json({ erro: "Username é obrigatório e deve ser uma string válida" });
    }
    ...
    if (deleted) {
      ...
      console.warn(
        "[USER CONTROLLER] Usuário não encontrado para deletar:",
        username
      );
