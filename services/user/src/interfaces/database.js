...
async function connectWithBackoff(attempts = 10) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      console.error(
        `[USER] Tentativa ${i}/${attempts} falhou ao conectar no Mongo:`,
        lastMongoErrorMsg
      );
...
export async function connectToDatabase() {
  ...
  if (!uri) {
    if (process.env.NODE_ENV === "production") {
      console.error(
        "[USER] MONGODB_URI is required in production. Aborting startup."
      );
      process.exit(1);
    } else {
      console.error(
        "[USER] MONGODB_URI missing - service will start but MongoDB is not connected"
      );
      return null;
    }
  }
  ...
  if (!ok) {
    console.error(
      "[USER] Falha inicial após várias tentativas. Continuará tentando em background a cada 30s."
    );
