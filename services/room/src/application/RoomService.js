...
export class RoomService {
  ...
  async createRoom(room) {
    ...
    if (room.number == null || room.type == null || room.price == null || room.capacity == null) {
      console.error('[RoomService] Dados do quarto incompletos:', room);
      throw new HttpError(400, 'Dados do quarto incompletos');
    }

    if (typeof room.number !== 'number' || room.number <= 0 || !Number.isInteger(room.number)) {
    ...
    try {
      ...
      if (existingRoom) {
        console.error('[RoomService] Número de quarto já cadastrado:', room.number);
      ...
      if (error?.code === 11000 || /E?11000|duplicate key/i.test(error?.message || '')) {
        throw new HttpError(409, 'Número de quarto já cadastrado');
      }
  ...
  async updateRoom(id, data) {
    ...
    if (data.number && (typeof data.number !== 'number' || data.number <= 0 || !Number.isInteger(data.number))) {
    ...
    try {
      ...
      if (!updatedRoom) {
        console.error('[RoomService] Quarto não encontrado para atualização:', id);
        throw new HttpError(404, 'Quarto não encontrado');
      }
      console.log('[RoomService] Quarto atualizado com sucesso:', updatedRoom);
      return updatedRoom;
    } catch (error) {
      if (error?.code === 11000) throw new HttpError(409, 'Número de quarto já cadastrado');
      console.error('[RoomService] Erro ao atualizar quarto:', error.message);
      throw (error instanceof HttpError ? error : new HttpError(500, `Erro ao atualizar quarto: ${error.message}`));
    }
  }
  ...
  async deleteRoom(id) {
    ...
    try {
      ...
      throw (error instanceof HttpError ? error : new HttpError(500, `Erro ao remover quarto: ${error.message}`));
    }
  }
  ...
  async updateRoomStatus(id, status) {
    ...
    try {
      console.log('[RoomService] Chamando setStatus no repositório com ID:', id);
      const updatedRoom = await this.roomRepository.setStatus(id, status);
      if (!updatedRoom) {
        console.error('[RoomService] Quarto não encontrado para atualização de status:', id);
        throw new HttpError(404, 'Quarto não encontrado');
      }
      console.log('[RoomService] Status do quarto atualizado com sucesso:', updatedRoom);
      return updatedRoom;
    } catch (error) {
      if (error.message.includes('Quarto não encontrado')) throw new HttpError(404, error.message);
      if (error.message.includes('ValidationError')) throw new HttpError(400, error.message);
      console.error('[RoomService] Erro ao atualizar status do quarto:', error.message);
      throw new HttpError(500, `Erro ao atualizar status do quarto: ${error.message}`);
    }
  }
