...
export class MongoRoomRepository extends IRoomRepository {
  async create(room) {
    console.log('[MongoRoomRepository] Tentando criar quarto:', { number: room?.number, type: room?.type });
    try {
  ...
  async findById(id) {
    ...
    try {
      ...
      console.log('[MongoRoomRepository] Quarto encontrado:', room ? { id: room._id, number: room.number } : null);
      return room ? { ...room.toObject(), id: room._id } : null;
    } catch (error) {
      console.error('[MongoRoomRepository] Erro ao buscar quarto por ID:', error.message);
      throw new HttpError(500, 'Erro interno do servidor');
    }
  }
  ...
  async findAll() {
    ...
    try {
      ...
      console.error('[MongoRoomRepository] Erro ao buscar todos os quartos:', error.message);
      throw new HttpError(500, 'Erro interno do servidor');
    }
  }

  async findByNumber(number) {
    console.log('[MongoRoomRepository] Tentando buscar quarto por número:', number);
    try {
      const room = await RoomModel.findOne({ number });
      console.log('[MongoRoomRepository] Quarto encontrado:', room ? { id: room._id, number: room.number } : null);
      return room ? { ...room.toObject(), id: room._id } : null;
    } catch (error) {
      console.error('[MongoRoomRepository] Erro ao buscar quarto por número:', error.message);
      throw new HttpError(500, 'Erro interno do servidor');
    }
  }

  async update(id, data) {
    console.log('[MongoRoomRepository] Tentando atualizar quarto:', { id, fields: Object.keys(data || {}) });
    try {
      const updatedRoom = await RoomModel.findByIdAndUpdate(id, data, { new: true, runValidators: true });
      if (!updatedRoom) {
        throw new HttpError(404, 'Recurso não encontrado');
      }
      console.log('[MongoRoomRepository] Quarto atualizado com sucesso:', { id: updatedRoom._id, number: updatedRoom.number });
      return { ...updatedRoom.toObject(), id: updatedRoom._id };
    } catch (error) {
      console.error('[MongoRoomRepository] Erro ao atualizar quarto:', error.message);
      if (error instanceof HttpError) throw error;
      if (error.code === 11000) throw new HttpError(409, 'Conflito: quarto já existe');
      if (error.name === 'ValidationError') throw new HttpError(400, 'Dados inválidos');
      throw new HttpError(500, 'Erro interno do servidor');
    }
  }
  ...
  async delete(id) {
    ...
    try {
      ...
      console.log('[MongoRoomRepository] Quarto removido com sucesso:', { id: deletedRoom._id, number: deletedRoom.number });
      return { ...deletedRoom.toObject(), id: deletedRoom._id };
    } catch (error) {
      console.error('[MongoRoomRepository] Erro ao remover quarto:', error.message);
      if (error instanceof HttpError) throw error;
      throw new HttpError(500, 'Erro interno do servidor');
    }
  }
  ...
  async setStatus(id, status) {
    console.log('[MongoRoomRepository] Atualizando status do quarto:', { id, status });
    try {
      ...
      console.log('[MongoRoomRepository] ID convertido para ObjectId:', objectId);
      ...
      console.log('[MongoRoomRepository] Status do quarto atualizado com sucesso:', { id: updatedRoom._id, status: updatedRoom.status });
      return { ...updatedRoom.toObject(), id: updatedRoom._id };
    } catch (error) {
      console.error('[MongoRoomRepository] Erro ao atualizar status do quarto:', error.message);
      if (error instanceof HttpError) throw error;
      if (error.name === 'ValidationError') throw new HttpError(400, 'Dados inválidos');
      throw new HttpError(500, 'Erro interno do servidor');
    }
